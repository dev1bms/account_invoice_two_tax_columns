...
### Test 1: Line with Tax 1 only
1. Create new Customer Invoice
2. Add a product line with price = 100
3. In "Tax 1" column, select your standard VAT (e.g., 15%)
4. Leave "Tax 2" column empty
5. **Expected**:
   - Tax 1 column shows 15% tax
...
1. Create new Customer Invoice
2. Add a product line with price = 100
3. Leave "Tax 1" column empty
4. In "Tax 2" column, select the 2.1% tax
5. **Expected**:
   - Tax 1 column is empty
...
4. "Tax 2" = 2.1% tax
5. **Expected**:
   - Tax 1 column shows 15%
   - Tax 2 column shows 2.1%
   - Total = 117.1 (100 + 15% + 2.1%)
6. Save draft

**Design Verification**: Check that tax_ids (native Odoo field, now hidden) contains both taxes:
- In debug mode, verify `line.tax_ids` contains both 15% and 2.1% taxes
...
2. On a line, select the same tax (e.g., 15%) in BOTH Tax 1 and Tax 2 columns
3. **Expected**:
...
**Important**: This constraint checks `tax1_ids` vs `tax2_ids`, NOT `tax_ids` vs `tax2_ids`.
...
2. Save draft
3. Change Tax 1 column to a different tax (e.g., 5%)
4. **Expected**:
   - Tax 2 column still shows 2.1%
   - Line effective taxes: Tax 1 = 5% + Tax 2 = 2.1%
   - Total updates correctly

**Design Verification**: Changing `tax1_ids` should not affect `tax2_ids`.

### Test 11: Remove Tax 2 updates correctly
1. Create invoice with line having Tax 1 (15%) + Tax 2 (2.1%)
2. Save draft
3. Clear the Tax 2 column
4. **Expected**:
   - Tax 2 column is now empty
   - Tax 1 column still shows 15%
   - Effective `tax_ids` now only contains 15%
   - Total = 115 (100 + 15%)

**Design Verification**: Clearing `tax2_ids` removes those taxes from effective `tax_ids`.
